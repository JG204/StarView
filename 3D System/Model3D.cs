...
namespace StarView
{
    ...
    {
        private ModelVisual3D ToRemove { get; set; }
        ...
        private void SetModel(ArrayList meshs, Material backMaterial, string sender, int itemNumber)
        {
            if (meshs == null)
            {
                return;
            ...
            for (int j = 0; j < nMeshNo; j++)
            {
                ...
                for (int i = 0; i < nVertNo; i++)
                {
                    ...
                    Point point;
                    if (sender != "dso" && j < itemNumber)
                    {
                        if (StarSorting.ListIndexes.ElementAt(j).ColorIndex == "")
                        {
                            point = new Point(0, 0);
                        }
                        else
                        {
                            double colorIndex = Convert.ToDouble(StarSorting.ListIndexes.ElementAt(j).ColorIndex, CultureInfo.InvariantCulture);
                            point = ColorIndexConverter.ColorIndexToPoint(colorIndex);

                        }
                    }
                    else
                    {
                        point = new Point(0.7, 0.7);
                    }

                    triangleMesh.TextureCoordinates.Add(new System.Windows.Point(point.X, point.Y));
        ...
        public int UpdateModel(ArrayList meshs, Material backMaterial, int nModelIndex, Viewport3D viewport3d, string sender, int itemNumber)
        {
            if (nModelIndex >= 0)
            {
                if (nModelIndex > 10)
                {
                    ToRemove = (ModelVisual3D)viewport3d.Children[nModelIndex];
                }

                viewport3d.Children.Remove(ToRemove);
            }
            ...
            SetModel(meshs, backMaterial, sender, itemNumber);
