...
namespace StarView
{
    public class TransformMatrix
    {
        ...
        public double m_scaleFactorSlow = 1.03;                      // sensativity for slow zoom
        ...
        {
            if (e.Delta > 0 /*&& e.MiddleButton != MouseButtonState.Pressed*/)
            {
                m_viewMatrix.Scale(new Vector3D(m_scaleFactor, m_scaleFactor, m_scaleFactor));
            }
            else if (e.Delta < 0 /*&& e.MiddleButton != MouseButtonState.Pressed*/)
            {
                m_viewMatrix.Scale(new Vector3D(1 / m_scaleFactor, 1 / m_scaleFactor, 1 / m_scaleFactor));
            }
            //else if (e.Delta > 0 && e.MiddleButton == MouseButtonState.Pressed)
            m_viewMatrix.Scale(new Vector3D(m_scaleFactorSlow, m_scaleFactorSlow, m_scaleFactorSlow));
            //else if (e.Delta < 0 && e.MiddleButton == MouseButtonState.Pressed)
            m_viewMatrix.Scale(new Vector3D(1 / m_scaleFactorSlow, 1 / m_scaleFactorSlow, 1 / m_scaleFactorSlow));
            m_totalMatrix = Matrix3D.Multiply(m_projMatrix, m_viewMatrix);
        }
